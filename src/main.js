...
import { loadTerminal, setRenderFunction, getTerminalLines } from './terminal/commands.js';
...
if (isDesktopDevice) {  
  ...
  import('./scene/scene.js').then(({ createScene, createCamera, createRenderer }) => {
    ...
    import('./scene/room.js').then(({ createWall, createTable }) => {
      createWall(scene);
      createTable(scene);
    });
    ...
    import('./scene/monitor.js').then(({ createMonitor }) => {
      ...
      import('./terminal/renderer.js').then(({ createScreenRenderer }) => {
        const { renderScreenTexture: renderFn } = createScreenRenderer(getTerminalLines(), screenMesh);
        ...
        import('./controls/mouse.js').then(({ setupMouseTracking }) => {
          ...
          function animate() {
            ...
            currentLookAt.x += (targetLookAt.x - currentLookAt.x) * LOOK_SENSITIVITY;
            currentLookAt.y += (targetLookAt.y - currentLookAt.y) * LOOK_SENSITIVITY;
  ...
  if (terminalBody) {
    import('./terminal/renderer-dom.js').then(({ createDomRenderer }) => {
      const { renderTerminal } = createDomRenderer(getTerminalLines(), terminalBody);
