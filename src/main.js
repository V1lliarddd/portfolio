...
if (isDesktopDevice) {
  ...
  import('./scene/scene.js').then(
    ({ createScene, createCamera, createRenderer }) => {
      ...
      // Камера дальше: z = 35 вместо 25
      camera.position.set(0, 3.0, 150);
      camera.lookAt(0, 0.4, 0);

      let startTime = Date.now();
      const duration = 3000; // 3 секунды для большего расстояния
      ...
      function animateCamera() {
        ...
        const startPos = new THREE.Vector3(0, 3.0, 35);
        camera.position.lerpVectors(startPos, finalPosition, ease);

        const lookY = 0.4 + (1 - ease) * 0.3;
